...
# assign get example
df <- data.frame(var1 = c("사과","딸기","바나나"), 
                 var2 = c("주스","아이스크림","우유"))

for (i in 1:length(df$var1)){
  assign(df$var1[i], df$var2[i]) # assign을 이용해서 var2의 데이터를 var1의 항목의 이름으로 변수 선언
}

for (i in 1:length(df$var1)){
  var <- get(df$var1[i]) #get을 이용해서 var1의 함목의 이름을 가진 변수의 데이터를 출력
  print(var)
}
...
fn <- "15_제어문/multi_sheet_read_exemple.xlsx"
vSh <- excel_sheets(fn)
