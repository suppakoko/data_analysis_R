...
## 힌트
빈도표를 만드는 `table()`과 결측치를 확인하는 `is.na()`를 조합해 보세요.
...
- Q2. `filter()`를 이용해 `hwy` 변수의 결측치를 제외하고, 어떤 구동방식의 `hwy` 평균이 높은지 알아보세요. (하나의 `dplyr` 구문으로 만들기)


## 힌트
`filter()`와 `is.na()`를 조합해 결측치를 제외하고, 집단별 평균을 구하는 코드를 `%>%`로 연결하면 됩니다.
...
## 힌트
`drv`가 정상적인 값이면 원래 값을 유지하고 그렇지 않으면 `NA`를 부여하는 코드를 작성하면 됩니다. 정상적인 값이 여러 개 있으니 `%in%`와 `c()`를 조합해 코드를 간결하게 만들어 보세요.



- Q2. 상자 그림을 이용해서 `cty`에 이상치가 있는지 확인하세요. 상자 그림의 통계치를 이용해 정상 범위를 벗어난 값을 결측 처리한 후 다시 상자 그림을 만들어 이상치가 사라졌는지 확인하세요.
## 힌트
상자 그림을 만들 때 사용하는 다섯 가지 통계치를 출력해 정상 범위의 기준을 찾으세요. 그런 다음 `filter()`를 이용해 `cty`가 이 범위를 벗어날 경우 `NA`를 부여하면 이상치가 결측 처리 됩니다.



- Q3. 두 변수의 이상치를 결측처리 했으니 이제 분석할 차례입니다. 이상치를 제외한 다음 `drv`별로 `cty` 평균이 어떻게 다른지 알아보세요. 하나의 `dplyr` 구문으로 만들어야 합니다.
## 힌트
`filter()`를 이용해 `drv`와 `cty`가 모두 결측치가 아닌 데이터를 추출한 후 집단별 평균을 구하면 됩니다.
...
Q3. 두 변수의 이상치를 결측처리 했으니 이제 분석할 차례입니다. 이상치를 제외한 다음 `drv`별로 `cty` 평균이 어떻게 다른지 알아보세요. 
