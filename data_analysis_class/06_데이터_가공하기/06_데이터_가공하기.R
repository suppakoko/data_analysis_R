...
## 힌트
특정 조건에 해당하는 데이터를 추출해서 평균을 구하면 해결할 수 있는 문제입니다. `filter()`를 이용해 `displ` 변수가 특정 값을 지닌 행을 추출해 새로운 변수에 할당한 다음 평균을 구해보세요.
...
## 힌트
`select()`로 변수를 추출해서 새로운 데이터를 만들어 보세요.
...
## 힌트
`filter()`로 조건에 해당하는 데이터를 추출한 뒤 평균을 구하면 해결할 수 있습니다.
...
## mpg 데이터에서 manufacturer, drv 별 cty 평균 상위 10 추출
...
#### 힌트
`group_by()`를 이용해 `class` 별로 나눈 뒤 `summarise()`를 이용해 `cty` 평균을 구하면 됩니다.
...
#### 힌트
앞에서 만든 코드를 `%>%`로 연결하고 내림차순으로 정렬하는 코드를 추가하면 됩니다.
...
#### 힌트
2번 문제와 같은 절차로 코드를 구성하고, 일부만 출력하도록 `head()`를 추가하면 됩니다.
...
##################################################################
## 데이터 가공하기 – 데이터 합치기 - left_join(), bind_row()
...
#### 힌트
`left_join()`을 이용해서 `mpg` 데이터에 `fuel` 데이터를 합치면 됩니다. 두 데이터에 공통으로 들어있는 변수를 기준으로 삼아야 합니다.
